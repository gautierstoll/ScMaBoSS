...
trait ResultProcessing {
  ...
  /** list of probability distribution with time
    ...
    * @return
    */
  val probDistTrajectory: List[(Double, Map[Set[String], Double])]
  val listNodes : List[String]
  ...
  /** list of size, useful for UPMaBoSS
    ...
    * @return
    */
  val sizes: List[Double] // careful: concrete class need to have same length with linesWithTime
...
class Result(val simulation: CfgMbss, verbose: Boolean, hexfloat: Boolean, outputData: String) extends ResultProcessing {
  val listNodes = simulation.extNodeList
